...
Detailed historical step numbering remains in the source repositories and retained evidence, but it is deliberately not the primary status language here.
...
1. **5A — readable current status** — active now; establish this page and the proposed next migration.
2. **5B — durable SCAD coordination conventions** — move only still-current architecture/working rules, not historical plans wholesale.
...
Tracking: issues #11, #25, #27, #30, #31 and #32.
...
- redesign SCons;
- implement physical-verification document bundles;
- introduce a second build engine;
- broadly migrate consumers.
